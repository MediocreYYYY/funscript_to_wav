...
class CONVERT:
    ...
    def __init__(self,framerate,minfra,maxfra):
        self.minfra=int(minfra)
        self.maxfra=int(maxfra)
    ...
    def processing(self,progressbar, bar):
        ...
        global d
        d=0
        m=int(math.sqrt(1024*1048576*100*1))
        ...
        #根据速度转换频率
        time_fra=np.clip(time_fra,1.25*min(time_fra),0.8*max(time_fra))
        ...
        R_strength=1-R_strength
        wave_data=(R_strength*R_a*np.sin(R_t*2*math.pi*R_fra)+L_strength*L_a*np.sin(L_t*2*math.pi*L_fra))*32767
  
        wave_data=np.floor(wave_data).astype(int)
        wave_data=np.clip(wave_data,-32767,32767)
